...
def mean_temp_accuracy(df: pd.DataFrame, mean_temp_min_name):
    mean_temp_accuracy_ = df.groupby('name')[['temp_max', 'temp_min']].mean().reset_index()
    mean_temp_accuracy_name = mean_temp_accuracy_.loc[mean_temp_accuracy_['name'] == mean_temp_min_name]
...
def mean_temp_max(df: pd.DataFrame, mean_temp):
...
def metrics(df: pd.DataFrame):
    ...
    name_mean_temp_accuracy, sub_mean_temp_accuracy = mean_temp_accuracy(df, name_mean_temp_min)
    name_mean_temp_max, round_mean_temp_max = mean_temp_max(df, mean_temp)
    name_mean_wind, round_mean_wind_max = mean_wind(df)

    return [name_mean_temp_min, round_mean_temp_min,
            name_mean_temp_accuracy, sub_mean_temp_accuracy,
